...
import { updateProductAction } from '@/src/actions/admin/products';
...
function sanitizeErrorMessage(error: any): string {
  ...
  if (msg.includes('Body exceeded') || msg.includes('limit')) {
    return 'Una de las fotografías es muy pesada. Intenta seleccionar fotos de menor resolución.';
  }
  if (msg.includes('No autorizado') || msg.includes('permisos') || msg.includes('JWT') || msg.includes('sesión')) {
    return 'Tu sesión de administrador ha caducado. Vuelve a iniciar sesión.';
  }
  if (msg.includes('fetch failed') || msg.includes('Failed to fetch')) {
    return 'Ocurrió un inconveniente de conexión con el servidor. Revisa tu conexión a internet.';
  }

  return msg || 'No se pudieron guardar los cambios en el producto.';
}
...
export default function AdminEditProductClient({
  ...
  // Tamaño
  const isPresetSize = ['Mini', 'Mediano', 'Grande'].includes(product.size);
  const [sizeSelect, setSizeSelect] = useState(isPresetSize ? product.size : 'Personalizado');
  const [customSize, setCustomSize] = useState(isPresetSize ? '' : product.size);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (sizeSelect === 'Personalizado' && !customSize.trim()) {
      setErrorMsg('Por favor especifica el tamaño personalizado.');
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 animate-in fade-in duration-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-500">
              Modifica los detalles, precios e imágenes asociadas a este amigurumi.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-stone-200/90 rounded-[12px] p-6 shadow-xs space-y-4">
            ...
            <div className="space-y-4">
              ...
              {sizeSelect === 'Personalizado' && (
                <div className="p-3.5 bg-amber-50/60 border border-amber-200/80 rounded-[8px] space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 bg-white border border-amber-300 rounded-[6px] text-xs text-stone-800 focus:outline-none focus:border-amber-700 font-medium"
                  />
                </div>
              )}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-[#206776] hover:underline"
                  >
                    <span>{showAddPresetForm ? 'Ocultar' : '+ Agregar nuevo material'}</span>
                  </button>
                </div>

                {showAddPresetForm && (
                  <div className="p-3 bg-teal-50/70 border border-teal-200/80 rounded-[8px] space-y-2">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="Ej: Cinta de raso, Lana Velvet..."
                        className="flex-1 px-3 py-1.5 bg-white border border-teal-300 rounded-[6px] text-xs text-stone-800 focus:outline-none"
                      />
                      <button
                        type="button"
                        onClick={() => handleAddPreset()}
                        className="px-3 py-1.5 bg-[#206776] text-white rounded-[6px] text-xs font-semibold"
                      >
                        Añadir
                      </button>
                    </div>
                  ...
                  {presetMaterials.map((mat) => {
                    ...
                    if (isConfirming) {
                      return (
                        <div
                          key={mat}
                          className="inline-flex items-center gap-2 px-3 py-1 bg-rose-50 border border-rose-300 text-rose-900 rounded-[6px] text-[11px] font-semibold"
                        >
                          <span>¿Eliminar "{mat}"?</span>
                          <button
                            type="button"
                            onClick={() => confirmRemovePreset(mat)}
                            className="px-2 py-0.5 bg-rose-600 text-white rounded-[4px] text-[10px] font-bold"
                          >
                            Sí
                          </button>
                          <button
                            ...
                            className="px-2 py-0.5 bg-stone-200 text-stone-700 rounded-[4px] text-[10px] font-bold"
                          >
                            No
                          </button>
                        </div>
                    ...
                    return (
                      <div
                        ...
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[6px] text-[11px] font-medium cursor-pointer transition-all ${
                          selected
                      ...
                      >
                        ...
                        <button
                          ...
                          className="ml-0.5 p-0.5 rounded-full hover:bg-stone-200/80"
                        >
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 bg-stone-50 border border-stone-200 rounded-[6px] text-xs text-stone-800 focus:outline-none focus:border-amber-700 resize-none font-mono text-[11px]"
                />
              </div>
            </div>
          </div>

          {/* GALERÍA DE FOTOS MULTI-IMAGEN */}
          <div className="bg-white border border-stone-200/90 rounded-[12px] p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <div>
                ...
                <p className="text-[11px] text-stone-400 mt-0.5">
                  Puedes agregar más fotos o reordenarlas. La primera será la foto de portada.
                </p>
              </div>
                ...
                >
                  ...
                  <div>
                    <p className="font-bold text-xs text-stone-800">
                      {isCompressing ? 'Optimizando imágenes...' : 'Haz clic para seleccionar o añadir más fotos desde tu equipo'}
                    </p>
                    <p className="text-[11px] text-stone-400 mt-0.5">
                      Soporta fotos PNG, JPG o WEBP de hasta 5MB por foto
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2 bg-stone-50 border border-stone-200 rounded-[6px] text-xs text-stone-800 placeholder-stone-400 focus:outline-none font-mono"
                  />
                  <button
                    type="button"
                    onClick={handleAddUrlImage}
                    className="px-3.5 py-2 bg-[#72594e] text-white font-semibold text-xs rounded-[6px] flex items-center gap-1 shrink-0"
                  >
                            ...
                            <div className="flex items-center justify-between">
                              {!isCover && (
                                <button
                                  ...
                                  className="px-2 py-0.5 bg-amber-500 text-white rounded text-[10px] font-bold flex items-center gap-0.5"
                              ...
                              <button
                                ...
                                className="ml-auto p-1 bg-rose-600 text-white rounded"
                                title="Eliminar foto"
                              >
        ...
        {/* COLUMNA DERECHA: PRECIOS, STOCK Y VISIBILIDAD */}
