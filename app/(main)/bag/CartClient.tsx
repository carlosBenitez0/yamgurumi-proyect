...
import DynamicConfetti from "react-confetti";
import { applyDiscountAction } from "@/src/actions/discounts/apply-discount";
import {
  getUserAvailableDiscountsAction,
  AvailableDiscount,
} from "@/src/actions/discounts/get-user-discounts";
import { createOrderAction } from "@/src/actions/orders/create-order";
...
export default function CartClient() {
  ...
  const [userDiscounts, setUserDiscounts] = useState<AvailableDiscount[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchDiscounts() {
      const res = await getUserAvailableDiscountsAction();
      if (isMounted && res.success && res.discounts) {
        setUserDiscounts(res.discounts);
      }
    }
    fetchDiscounts();
    return () => {
  ...
  const handleApplyCodeString = async (codeToApply: string) => {
    if (!codeToApply.trim()) return;
    setCouponState({ loading: true });
    ...
    try {
      ...
      if (result.success && result.percent !== undefined) {
        ...
        setTimeout(() => setConfettiFading(true), 6000);
      ...
      console.error(err);
      setCouponState({ loading: false, error: "Error al aplicar el cupón." });
    }
  };

  const handleApplyCoupon = async (e: React.FormEvent) => {
    e.preventDefault();
    handleApplyCodeString(couponInput);
  };
  ...
  const handleSend = async () => {
    ...
    if (!form.name.trim())
      nextErrors.name = "Escribe tu nombre para que el taller te reconozca.";
    if (!form.phone.trim())
  ...
  return (
    <main className="min-h-screen pb-28 lg:pb-16">
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-body-sm text-on-surface-variant font-body mt-1.5">
              {count} {count === 1 ? "pieza" : "piezas"} · {items.length}{" "}
              {items.length === 1 ? "amigurumi" : "amigurumis"}
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_380px] gap-6 lg:gap-10 mt-6 sm:mt-8 items-start">
          ...
          <div className="flex flex-col gap-6 self-start w-full">
            ...
            >
              ...
              {discountPercent > 0 && (
                <div className="flex items-center justify-between mt-2 text-secondary font-body">
              ...
              {/* Formulario y selección de cupones */}
              <div className="mt-4 pt-4 border-t border-outline-variant/15">
                {discountCode ? (
                  <div className="flex items-center justify-between bg-secondary-container/20 border border-secondary-container/40 rounded-2xl px-4 py-2.5">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-secondary block">
                        Cupón activo
                      </span>
                      <strong className="text-sm font-headline text-secondary">
                        {discountCode} (-{discountPercent}%)
                      </strong>
                    </div>
                    <button
                      onClick={removeDiscount}
                      className="text-xs font-body font-bold text-error hover:underline cursor-pointer"
                    >
                      Quitar
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {userDiscounts.length > 0 && (
                      <div className="bg-gradient-to-r from-secondary-container/30 to-primary-container/20 border border-secondary/35 rounded-2xl p-3 flex items-center justify-between gap-2.5 shadow-sm">
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="w-9 h-9 rounded-xl bg-secondary/15 text-secondary flex items-center justify-center shrink-0 border border-secondary/25 shadow-sm">
                            <MdConfirmationNumber className="text-xl" />
                          </div>
                          <div className="min-w-0">
                            <span className="text-xs font-bold text-on-surface block font-headline leading-tight truncate">
                              {userDiscounts.length}{" "}
                              {userDiscounts.length === 1
                                ? "cupón disponible"
                                : "cupones disponibles"}
                            </span>
                            <span className="text-[10px] text-secondary font-semibold block truncate">
                              Aplica tu descuento Yamgurumi
                            </span>
                          </div>
                        </div>
                        <button
                          ...
                          className="px-3.5 py-1.5 rounded-xl bg-secondary text-white text-xs font-bold shadow-button hover:bg-secondary/90 transition-all tactile-press shrink-0 cursor-pointer active:scale-95"
                        >
                          Ver lista
                        </button>
                      </div>
                    ...
                    <form onSubmit={handleApplyCoupon} className="flex gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setCouponState({ loading: false });
                        }}
                        className="bg-surface-container border border-outline-variant/30 rounded-xl px-3 py-2 text-xs font-body outline-none focus:ring-1 focus:ring-secondary w-full"
                      />
                      <button
                        type="submit"
                        disabled={couponState.loading || !couponInput.trim()}
                        className="bg-secondary text-white font-bold px-4 py-2 rounded-xl text-xs hover:bg-secondary/90 transition-colors disabled:opacity-50 whitespace-nowrap cursor-pointer"
                      >
                        Aplicar
                      </button>
                    </form>
                ...
                {couponState.error && (
                  <p className="text-[11px] text-error font-body mt-1.5 text-left">
                    {couponState.error}
                  </p>
                )}
                {couponState.success && (
                  <p className="text-[11px] text-secondary font-body mt-1.5 text-left">
                    {couponState.success}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col gap-4 mt-5">
                <div>
                  ...
                  <input
                    ...
                    aria-required="true"
                    aria-describedby={
                      errors.name ? "cart-name-error" : undefined
                    }
                  />
                  {errors.name && (
                    <p
                ...
                <div>
                  ...
                  <input
                    ...
                    aria-required="true"
                    aria-describedby={
                      errors.phone ? "cart-phone-error" : undefined
                    }
                  />
                  {errors.phone && (
                    <p
      ...
      {/* ── Confetti por cupón válido (Desvanecimiento suave) ── */}
      {showConfetti && (
        <div
      ...
      {/* ── Modal de selección de cupones de descuento ───────── */}
      <CouponSelectorModal
        open={isCouponModalOpen}
        onClose={() => setIsCouponModalOpen(false)}
        discounts={userDiscounts}
        activeDiscountCode={discountCode}
        onApplyCoupon={handleApplyCodeString}
